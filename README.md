...
5. Ejecuta `pnpm run dev` y prueba el flujo:
   ...
   - di «quiero explicártelo yo»: el tutor fija los puntos clave que debes cubrir (sin mostrarte la solución); en el panel, graba (reconocimiento de voz del navegador) o escribe tu explicación y corrígela: cada punto queda cubierto, a medias, falta o incorrecto, con lo esperado en los que no cubriste y la página que lo explica; pregúntale al tutor por un punto y repite,
...
- El dictado usa el reconocimiento de voz del navegador (Chrome, Edge y Safari; Firefox no lo tiene y deja solo el texto); en Chrome y Edge el audio pasa por el servicio de Google. No se guarda audio en ningún caso. Con `VITE_EXPLAIN_DICTATION=simulated` en `.env` el micro reproduce en su lugar una muestra generada en el servidor a partir de las soluciones (buena / a medias / floja), para demos sin micrófono; el endpoint de muestras es de demostración y expone las soluciones a quien lo llame.
...
- La ruta de streaming del chat es manual (fuera de Effect HTTP API), como en la base original. Los tokens de la respuesta llegan en vivo (`text-delta`), pero los pasos previos del turno (leer páginas, crear un artefacto) se muestran como progreso, no como texto.
