...
- El dictado es simulado por defecto: el micro reproduce una muestra generada en el servidor a partir de las soluciones (buena / a medias / floja); el endpoint de muestras es de demostración y expone las soluciones a quien lo llame. Con `VITE_EXPLAIN_DICTATION=browser` en `.env` el micro usa el reconocimiento de voz del navegador (Chrome, Edge y Safari; Firefox no lo tiene); en Chrome y Edge el audio pasa por el servicio de Google. No se guarda audio en ningún caso.
