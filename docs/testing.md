...
6. Resolver: contesta con un fallo y envía. Puntuación, corrección por pregunta con explicación y reintento. Volver a pulsar una opción marcada la desmarca (el contador de respondidas baja). El icono de ampliar en la cabecera del panel abre la sesión de estudio a pantalla completa en una columna de lectura, conservando las respuestas; «Volver» o Esc regresan al panel sin cerrarlo (en móvil no aparece; el esquema conserva su propio «Ampliar»).
...
6c. Explicación: «Quiero explicarte yo el ciclo del agua y que me corrijas» → el tutor lee las páginas si hace falta, crea el objetivo y el chat muestra «Explicación · 4 puntos clave · Abrir». En el panel: el enunciado y los títulos de los puntos (nunca lo esperado); en Chrome o Edge, «Grabar» pide permiso de micro y muestra «Escuchando · 0:05» con el texto apareciendo mientras hablas; «Parar» deja el texto editable; «Corregir» → anillo de nota, cada punto con Cubierto / A medias / Falta / Incorrecto, comentario, «Lo esperado» solo en los no cubiertos, chips de página con vista previa y «Preguntar»; abajo, tu texto con los fragmentos que activaron cada punto subrayados. «Preguntar» en un punto rellena el chat y el turno lleva `openQuestionId`; el tutor explica lo que faltó citando la página. «Repetir» y explícalo mejor → «Antes: …». También puedes escribir en lugar de grabar. Deniega el permiso de micro: aviso en rojo con cómo permitirlo y el textarea sigue disponible. En Firefox el micro queda desactivado con el motivo. Sin modelo: copia el JSON de `docs/api.md` a `.data/artifacts/artifacts/<id>.json` con un `materialId` existente y abre el panel.
6c-bis. Dictado simulado (demo sin micrófono): pon `VITE_EXPLAIN_DICTATION=simulated` en `.env`, reinicia `pnpm run dev` y abre la explicación: aparece el selector de muestras (buena / a medias / floja) y la nota indica que es una simulación; «Grabar» con «a medias» dicta el texto palabra a palabra y «Corregir» da una nota parcial; «Repetir» con «buena» → 4/4 y «Antes: …».
6d. Tutorías (prototipo, sin modelo): edita la carpeta (lápiz) y elige UCM → Biología → Ecología; bajo el selector aparece el chip «UCM · Biología · 3.º · Ecología», en la barra la sección «Tutorías» («3 tutores de tu grado» · 120 puntos) y en el chat vacío la sugerencia «¿Prefieres que te lo explique alguien de tu grado?». Ábrela: la lista ordena a los tutores por coincidencia con la práctica de la carpeta («Ciclo del agua» en amarillo, «Recomendada para ti»). «Ver perfil» → franjas, política y reseñas; «Reservar por 40 puntos» → hoja con duración, qué trabajar (prellenado), qué compartir y coste `120 → 80`; «Reservar» → confirmación con «Marcar como realizada» y «Cancelar reserva» (devuelve los puntos); la barra pasa a «Mañana 18:00 · Marta R.». Marca la sesión como realizada → tarjeta de reseña (estrellas, etiquetas, texto) → «Publicar reseña» aparece la primera en el perfil como «sesión verificada». Corrige un quiz con menos del 50 %: bajo la nota sale el gancho «Ver tutores de Ecología para este tema →», que abre la lista con ese tema primero. Recarga: la reserva y los puntos se conservan (`localStorage`). Quita la asignatura (los tres selectores vacíos + Guardar): la sección pide indicarla y el gancho desaparece.
