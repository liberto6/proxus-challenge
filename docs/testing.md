...
6c-bis. Dictado real: pon `VITE_EXPLAIN_DICTATION=browser` en `.env`, reinicia `pnpm run dev` y abre la explicación en Chrome o Edge: desaparece el selector de muestras, la nota indica que reconoce el navegador, «Grabar» pide permiso de micro y muestra «Escuchando · 0:05» con el texto apareciendo mientras hablas; «Parar» deja el texto editable y «Corregir» funciona igual. Deniega el permiso: aviso en rojo con cómo permitirlo y el textarea sigue disponible. En Firefox el micro queda desactivado con el motivo.
