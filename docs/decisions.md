...
- **Añadido.** El dictado real existe como un segundo hook con la misma interfaz (`Dictation`) sobre la Web Speech API del navegador, sin clave ni servidor. Ambas fuentes se montan siempre y el panel habla con la configurada; en un navegador sin reconocimiento el micro explica por qué y queda el texto.
- **Revisado (2026-09-25).** El reconocimiento del navegador pasa a ser el valor por defecto; el simulado queda solo tras `VITE_EXPLAIN_DICTATION=simulated`, para demos sin micrófono. Motivo: al probar el producto con la simulación por defecto, el micro parecía falso; lo que se evalúa es la voz real, y la simulación era una muleta de desarrollo. No hay cascada automática entre fuentes: sin reconocimiento en el navegador queda el texto.
